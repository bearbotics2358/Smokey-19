...
TrajectoryInfo TrajectoryCalc::compute_trajectory(TrajectoryInfo inputs) 
{
	...
	inputs.t = (units::second_t)(data.t[m_model][(int)inputs.elevation_angle.value()][get_rpm_index(inputs.wheel_rpm)]);
...
{
	int indx = 0;
	double rpm_max = RPM_BASE + (TRAJECTORY_TABLE_SPEEDS - 1) * RPM_INC;
	
	if(rpm > rpm_max) {
		rpm = rpm_max;
	}
	
	indx = (rpm - RPM_BASE) / (1.0 * RPM_INC);
...
units::revolutions_per_minute_t TrajectoryCalc::get_rpm(int rpm_index) {
	...
	ret = (units::revolutions_per_minute_t)(RPM_BASE + rpm_index * RPM_INC);
	return ret;
}

/*
units::feet_per_second_t TrajectoryCalc::fuel_velocity(units::revolutions_per_minute_t rpm) {
